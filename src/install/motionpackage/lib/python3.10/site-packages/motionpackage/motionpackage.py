...
class MyNode(Node):
    ...
    def SectorSend2FPGAFuction(self, msg):
        ...
        try:
            ...
            if self.SendSectorPackage[0] == 241:
                ...
                self.ExecuteCallBack_Publish.publish(execute_ack)
        ...
        if(
            ...
            with open(path, "w") as OutFile:
                ...
                if self.SaveSectorPackage[2] in [241, 242, 243]:
                    ...
                    self.interfaceSend2Sector.publish(self.interface_ack)
    ...
    def mcssl_init(self):
        ...
        if tool.stand_path == "/home/iclab/Desktop/Standmotion":
            self.devs = "/dev/ttyUSB0"
            self.devs_head = "/dev/ttyS1"
            self.devs_IMU = "/dev/ttyS0"
            print("aaaaaa")
            print(tool.stand_path)
        else:
            self.devs = "/dev/ttyUSB1"
            self.devs_head = "/dev/ttyUSB2"
            self.devs_IMU = "/dev/ttyUSB0"
            print("bbbbb")
            print(tool.stand_path)
        self.baudrate = 115200
        self.bytesize = 8
        # 初始化串口变量
        # Initialize serial ports and perform the necessary operations
        try:
            self.serial      = serial.Serial(self.devs,       baudrate=self.baudrate, bytesize=self.bytesize, timeout=1)
            self.serial_head = serial.Serial(self.devs_head,  baudrate=self.baudrate, bytesize=self.bytesize, timeout=1)
            self.serial_IMU  = serial.Serial(self.devs_IMU,   baudrate=self.baudrate, bytesize=self.bytesize, timeout=1)

            print(f"Initialize Motion with port={self.devs}...")
            print(f"Initialize Head with port={self.devs_head}...")
            print(f"Initialize IMU with port={self.devs_IMU}...")

            # Check if serial ports were successfully opened and set flow control as needed
            if  self.serial is None:
                print(f"Motion RS232 OPEN FAIL: {self.serial.port}")
                return -1

            if  self.serial_head is None:
                print(f"Head RS232 OPEN FAIL: {self.serial_head.port}")
                return -1

            if not self.serial_IMU is None:
                print(f"IMU RS232 OPEN FAIL: {self.serial_IMU.port}")
                return -1

            self.serial.setRTS(False)
            self.serial.setDTR(False)
            self.serial_head.setRTS(False)
            self.serial_head.setDTR(False)
            self.serial_IMU.setRTS(False)
            self.serial_IMU.setDTR(False)
    ...
    def head_callback(id, buf, length):
        pass
        # 在这里执行回调函数的操作，处理传入的参数
        # id 是整数
        # buf 是 bytearray 或类似的字节数组
        # length 是整数
    ...
    def IMU_callback(self,id, buf, length):
        ...
        if len(self.IMUPackage) >= IMU_PACKAGE_SIZE:
            for i in range(len(self.IMUPackage) - IMU_PACKAGE_SIZE + 1):
                if (
                    self.IMUPackage[i] == 0x53
                    and self.IMUPackage[i + 1] == 0x54
                    and self.IMUPackage[i + 2] == 0xF7
                    and self.IMUPackage[i + IMU_PACKAGE_SIZE - 1]
                ):
                    index = i
            ...
            self.IMUPackage.clear()  # 清空列表，以便存储下一个包

    def mcssl_callback(self,id, buf, length):
        print("FPGAack is: 0x%x 0x%x 0x%x 0x%x" % (buf[0], buf[1], buf[2], buf[3]))
        for i in range(4):
            if self.dio_tmpstatus == 0:
                ...
                FPGAack_msg = UInt8()
                FPGAack_msg.data = buf[i]
                self.FPGAack_Publish.publish(FPGAack_msg)
            elif dio_tmpstatus == 3:
                if buf[i] == ord('E'):
                    dio_tmpstatus = 0
                    if self.walkdata_receive:
                        walkack_msg = UInt8()
                        walkack_msg.data = 1
                        self.walkack_Publish.publish(walkack_msg)
            elif dio_tmpstatus == 4:
                if buf[i] == ord('Y'):
                    InterfaceFlag = 1
                elif buf[i] == ord('N'):
                    InterfaceFlag = 0
...
##########################################################################
    ...
    def run(self):
        ...
        # Logic for parameterPath
        self.parameterPath = location  # Depending on how 'parameterPath' is declared
        ...
        self.get_logger().info('Motion is running')
        time.sleep(1)

        self.create_timer(1.0 / 60, self.loop_function)  # Run at 60 Hz
    ...
    def loop_function(self):
        if self.read_IMU_count >= 3:  # 60/4 = 15 Hz
